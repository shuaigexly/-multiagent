    ...
    return [
        ...
        },
        {
            ...
            "body": {
                ...
                "title": "A4 复核提醒",
                "steps": [
                    _reminder_trigger("step_trigger", "建议复核时间", "建议复核时间到达"),
                    _set_record_action(
                        "step_stage",
                        "分析任务",
                        [
                    ...
                    _add_record_action(
                        ...
                        [
                            ...
                            _action_status_field("动作状态", "待执行"),
                            _route_field("工作流路由", "补数复核"),
                            _text_field("动作内容", "请按主表里的需补数条数、异常说明和复核 SLA 继续处理。"),
                            _text_field("执行结果", "自动化 scaffold 已创建复核动作，请在飞书里绑定具体负责人和复核清单。"),
                            _ref_field("关联记录ID", "$.step_trigger.recordId"),
                        ],
                        next_step="step_notify",
                    ),
    ...
    return [
        ...
        },
        {
            ...
            "body": {
                ...
                "title": "W3 执行分支工作流",
                "steps": [
                    _checkbox_trigger("step_trigger", "待执行确认", "进入执行队列"),
                    _set_record_action(
                        "step_stage",
                        "分析任务",
                        [
...
def _select_trigger(step_id: str, field_name: str, option_name: str, title: str) -> dict[str, Any]:
    return {
        "id": step_id,
        "type": "SetRecordTrigger",
        "title": title,
...
def _reminder_trigger(step_id: str, field_name: str, title: str) -> dict[str, Any]:
    return {
        "id": step_id,
