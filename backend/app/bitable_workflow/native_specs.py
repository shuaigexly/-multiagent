...
def build_automation_specs() -> list[dict[str, Any]]:
    return [
        ...
        {
            "name": "A2 直接汇报自动提醒",
            "summary": "直接汇报任务满足汇报条件时，自动切换到汇报责任面，并写一条自动化承接日志。",
            "trigger": "字段修改时",
            "condition": "待发送汇报 = 是 且 工作流路由 = 直接汇报",
            "action": "回写汇报责任角色 + 发送管理提醒 + 记录自动化承接",
            "primary_field": "工作流消息包",
            "native_goal": "用自动化承接直接汇报入口，具体汇报动作交给 W2 工作流统一创建，避免重复动作。",
            ...
            "body": {
                ...
                "steps": [
                    ...
                    _add_record_action(
                        ...
                        "自动化日志",
                        [
                            _text_field("日志标题", "A2 直接汇报自动提醒"),
                            _text_field("任务标题", "请在主表查看对应分析任务"),
                            _text_field("节点名称", "A2 直接汇报自动提醒"),
                            _text_field("触发来源", "automation.report_direct"),
                            _action_status_field("执行状态", "已完成"),
                            _text_field("日志摘要", "直接汇报入口已由原生自动化承接，后续动作交给 W2 工作流创建。"),
                            _text_field("详细结果", "请在飞书工作流里继续完成汇报动作、接收人绑定和反馈回写。"),
                            _ref_field("关联记录ID", "$.step_trigger.recordId"),
                        ],
                    ),
                ],
        ...
        {
            "name": "A3 执行任务自动创建",
            "summary": "执行分支触发后，自动切换到执行责任面，并写一条执行入口日志。",
            "trigger": "字段修改时",
            "condition": "待创建执行任务 = 是",
            "action": "回写执行责任角色 + 提醒执行负责人 + 记录自动化承接",
            "primary_field": "工作流执行包",
            "native_goal": "把执行入口交给自动化先承接，具体执行动作由 W4 工作流统一创建，避免重复执行单。",
            ...
            "body": {
                ...
                "steps": [
                    ...
                    _message_action(
                        ...
                        [
                            "执行入口已经由自动化承接。",
                            "请在主表查看执行负责人、执行截止时间和执行包，后续执行动作由 W4 工作流创建。",
                        ],
                        next_step="step_log",
                    ),
                    _add_record_action(
                        ...
                        "自动化日志",
                        [
                            _text_field("日志标题", "A3 执行任务自动创建"),
                            _text_field("任务标题", "请在主表查看对应分析任务"),
                            _text_field("节点名称", "A3 执行任务自动创建"),
                            _text_field("触发来源", "automation.execution_entry"),
                            _action_status_field("执行状态", "已完成"),
                            _text_field("日志摘要", "执行入口已由原生自动化承接，后续执行动作交给 W4 工作流创建。"),
                            _text_field("详细结果", "请在飞书工作流里继续补齐执行任务、负责人绑定和截止时间。"),
                            _ref_field("关联记录ID", "$.step_trigger.recordId"),
                        ],
                    ),
                ],
        ...
        {
            "name": "A4 复核提醒",
            "summary": "复核时间到达后，自动切换到复核责任面，并写一条到期提醒日志。",
            "trigger": "到达记录中的时间时",
            "condition": "待安排复核 = 是 且 建议复核时间到达",
            "action": "回写复核责任角色 + 提醒复核负责人 + 记录到期提醒",
            "primary_field": "建议复核时间",
            "native_goal": "让时间到达提醒和复核动作拆层，真正创建复核动作交给 W5 工作流，避免重复动作。",
            ...
            "body": {
                ...
                "steps": [
                    ...
                    _message_action(
                        ...
                        [
                            "复核到期提醒已经触发。",
                            "请按证据链、评审记录和复核历史继续补数或重跑，具体复核动作由 W5 工作流创建。",
                        ],
                        next_step="step_log",
                    ),
                    _add_record_action(
                        ...
                        "自动化日志",
                        [
                            _text_field("日志标题", "A4 复核提醒"),
                            _text_field("任务标题", "请在主表查看对应分析任务"),
                            _text_field("节点名称", "A4 复核提醒"),
                            _text_field("触发来源", "automation.review_due"),
                            _action_status_field("执行状态", "已完成"),
                            _text_field("日志摘要", "复核到期提醒已触发，具体复核动作交由 W5 工作流统一承接。"),
                            _text_field("详细结果", "请在飞书工作流里继续处理复核任务、补数分支和重跑分支。"),
                            _ref_field("关联记录ID", "$.step_trigger.recordId"),
                        ],
                    ),
                ],
