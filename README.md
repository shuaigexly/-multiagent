...
> 默认 Base 采用紧凑工作流视图：保留 `🧭 工作流路由 / ⏳ 待拍板确认 / 🚀 待执行落地 / 🗓 待安排复核 / 🔁 待进入复盘 / 🟥 已异常任务` 等高频入口，健康度、优先级、证据用途等低频切片回到字段筛选和仪表盘里处理，避免多维表格打开后被几十个视图淹没。
...
角色化驾驶舱与多维表格视图优先消费这些字段，而不是依赖前端自行推断状态。每轮调度还会回刷已完成任务的责任归属和超期异常，让 `🧭 工作流路由 / ⏳ 待拍板确认 / 🚀 待执行落地 / 🗓 待安排复核 / 🟥 已异常任务` 等原生视图可以直接作为飞书汇报与自动化入口。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/v1/workflow/setup` | 一键创建 12 张表、精简工作流视图、模板中心与种子任务 |
...
| `GET`  | `/api/v1/workflow/telemetry` | 综合遥测：runner 状态 / 多租户注册表（脱敏） / LLM 用量（task / tenant / global，含 reasoning） / SSE 订阅器规模 / **每岗熔断器状态** |
...
```bash
# 1. 初始化（创建 12 张表、精简工作流视图、模板中心 + 写入种子任务）
