...
- `automation` pack 已经拆成 9 条自动化 scaffold
  - A1 新任务入场提醒
  - A2 直接汇报自动提醒
  - A2b 待拍板汇报提醒
  - A3 执行任务自动创建
...
- 消息接收人、审批人、执行人绑定
- 工作流里更深的审批 / 任务 / AI 节点
- `W8` 真实接群消息仍依赖外部消息解析器先把命令写入 `自动化日志.im.command`
- `W9` 当前默认在任务归档时触发，若要改成固定时段推送，仍需在飞书侧追加调度器
- 仪表盘最终排版
- 角色成员分配
