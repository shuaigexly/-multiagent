...
export function EmptyState({ text }: { text: string }) {
  return <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50/80 px-3 py-4 text-sm leading-6 text-slate-500">{text}</div>;
}
...
export function ResolutionCard({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white/92 p-3">
      ...
      <div className="mt-3 space-y-2">
        {nodes.map((node, index) => (
          <div key={node.key} className="relative pl-8">
            ...
            <div className={`absolute left-0 top-1 h-7 w-7 rounded-md border ${TRACE_NODE_STYLE[node.tone]}`} />
